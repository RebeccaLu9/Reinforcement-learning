...
class QLearningAgent:
    ...
    def __init__(self, game, discount, learning_rate, explore_prob):
        ...
        self.q_table = {} #mapping from (s,a) --> Q value
    ...
    def get_q_value(self, state, action):
        ...
        """
        inp = (state,action)
        if inp in self.q_table.keys():
            return self.q_table[inp]
        return 0.0  
    ...
    def get_value(self, state):
        ...
        for action in self.game.get_actions(state):
            if self.q_table[(state, action)] > output:
                output = self.q_table[(state, action)]
        return output # TODO
        '''
        op = []
        for action in self.game.get_actions(state):
    ...
    def get_best_policy(self, state):
        ...
        return random.choice(best_policy)
    ...
    def update(self, state, action, next_state, reward):
        ...
        """
        #pass
        self.q_table[(state,action)] = (1-self.learning_rate) * self.get_q_value(state,action) +\
        self.learning_rate * (reward + self.discount*self.get_value(next_state))
    ...
    def get_action(self, state):
        ...
        """
        if (random.random() < self.explore_prob): #random.random() is between 0 and 1
            #with epsilon prob
...
class ApproximateQAgent(QLearningAgent):
    ...
    def get_weight(self, feature):
        ...
        if feature in self.weight_table.keys():
...
feedback_question_2 = """
This module is hard to understand.
"""
